"use client";
import { motion } from "framer-motion";
import { Logo } from "./Logo";
import { Navigation } from "./Navigation";
import { VisitButton } from "./VisitButton";
import MobileMenu from "./side-menu/MobileMenu";

interface FixedHeaderProps {
  isVisible: boolean;
  activeMenu: "main" | "fixed" | null;
  onMenuToggle: () => void;
}

export const FixedHeader = ({ isVisible, activeMenu, onMenuToggle }: FixedHeaderProps) => {
  const showHeader = isVisible;

  return (
    <motion.header
      initial={{ y: "-100%" }}
      animate={{ y: showHeader ? 0 : "-100%" }}
      transition={{ duration: 0.35, ease: "easeInOut" }}
      className="z-20 fixed top-0 flex w-full h-24 bg-deepBlack backdrop-blur-sm items-center shadow-lg"
      onClick={() => activeMenu === "fixed" && onMenuToggle()}
    >
      <div className="w-full max-w-7xl mx-auto px-8 grid grid-cols-12 items-center">
        <div className="flex col-span-6 md:col-span-2">
          <Logo className="transition-transform duration-300 hover:scale-105" />
        </div>

        <Navigation/>

        <div className="flex items-center justify-end md:col-span-2 col-span-6 gap-4">
          <VisitButton />
          <div className="md:hidden">
            <MobileMenu isActive={activeMenu === "fixed"} setIsActive={onMenuToggle} />
          </div>
        </div>
      </div>
    </motion.header>
  );
};
